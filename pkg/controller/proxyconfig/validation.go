	...
	if isSpecReadinessEndpointsSet(proxyConfig) {
		...
		for _, endpoint := range proxyConfig.ReadinessEndpoints {
			...
			case scheme == proxyHTTPScheme:
				if !isSpecHTTPProxySet(proxyConfig) {
					return fmt.Errorf("httpProxy must be set when using a http readinessEndpoint")
				}
				if err := validateReadinessEndpoint(caBundle, proxyConfig.HTTPProxy, endpoint); err != nil {
					return fmt.Errorf("readinessEndpoint probe failed for endpoint '%s': %v", endpoint, err)
				}
			case scheme == proxyHTTPSScheme:
				if !isSpecHTTPSProxySet(proxyConfig) {
					return fmt.Errorf("httpsProxy must be set when using a https readinessEndpoint")
				}
				caBundle, _, _, err := r.validateTrustedCA(names.TRUSTED_CA_BUNDLE_CONFIGMAP)
				if err != nil {
					return fmt.Errorf("failed to validate TrustedCA %q: %v", proxyConfig.TrustedCA.Name, err)
				}
				if err := validateReadinessEndpoint(caBundle, proxyConfig.HTTPSProxy, endpoint); err != nil {
					return fmt.Errorf("readinessEndpoint probe failed for endpoint '%s': %v", endpoint, err)
				}
			default:
...
func (r *ReconcileProxyConfig) validateTrustedCA(trustedCA string) ([]*x509.Certificate, []byte, []byte, error) {
	...
	if err != nil {
		return nil, nil, nil, fmt.Errorf("failed to validate configmap reference for proxy trustedCA '%s': %v",
			trustedCA, err)
	}

	caBundle, bundleData, err := r.validateTrustBundle(cfgMap)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("failed to validate trust bundle for proxy trustedCA '%s': %v",
			trustedCA, err)
	}

	systemData, err := r.validateSystemTrustBundle(names.SYSTEM_TRUST_BUNDLE)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("failed to validate system trust bundle '%s': %v", names.SYSTEM_TRUST_BUNDLE, err)
	}

	return caBundle, bundleData, systemData, nil
}
...
func runReadinessProbe(caBundle []*x509.Certificate, proxy, endpoint *url.URL) error {
	...
	if err != nil {
		return fmt.Errorf("readiness probe failed for endpoint '%s' using proxy '%s': %v",
